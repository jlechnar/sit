...
def cmd_st(**parameters):
    the_sit.sit_status(parameters)
def cmd_stat(**parameters):
    the_sit.sit_status(parameters)
...
if __name__ == '__main__':
    ...
    if the_sit.is_svn_path():
        ...
        if sys.version_info[0] >= 3:
          parser_sit_commit = subparsers.add_parser('commit', aliases=['ci'], help='commit changes of the top relative to the local path, or for the given path')
        else:
        ...
        ########################
        if sys.version_info[0] >= 3:
          parser_sit_status = subparsers.add_parser('status', aliases=['stat', 'st'])
        else:
        ...
        ########################
        if sys.version_info[0] >= 3:
          parser_sit_update = subparsers.add_parser('update', aliases=['up'], help="Update path or complete sandbox")
        else:
